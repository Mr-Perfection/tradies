class PostAttachmentsController < ApplicationController
  ...
  before_action :logged_in_user, only: [:show ,:edit, :new , :create, :update, :destroy]
  before_action :correct_user,   only: [:edit, :update, :new , :create]
  before_action :save_previous_page, only: [:show]
  # before_action :check_limit, only: [:edit, :create] # you could add here: :only => [:index, :new]
  ...
  def show
    @back_button = session.delete(:return_to)
  end
  ...
  def update
    respond_to do |format|
      if @post_attachment.update_attributes(post_attachment_params)
        flash[:success] = "Post attachment was successfully updated."
        # redirect_to @post_attachment.post 
        format.html { redirect_to @post_attachment.post}
  ...
  private
    ...
    def correct_user
      post_attachment = PostAttachment.find(params[:id])
      @user = User.find(post_attachment.post.user)
